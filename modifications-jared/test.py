...
while not done:
    try:
        ...
        errorFrame = vo.id
        print(f"-------------------------Error on frame {errorFrame}-------------------------")
        pathlib.Path.unlink(f"D:\\School\\S7\\robots-temp\\pngs\\{str(errorFrame - 1).zfill(6)}.png")
        pathlib.Path.unlink(f"D:\\School\\S7\\robots-temp\\pngs\\{str(errorFrame).zfill(6)}.png")
        os.chdir('D:\\School\\S7\\robots-temp\\pngs')
        
        for count, f in enumerate(os.listdir()):
            f_name, f_ext = os.path.splitext(f)
            f_name = str(count).zfill(6)
        
            new_name = f'{f_name}{f_ext}'
            os.rename(f, new_name)
        vo = MonoVideoOdometery(img_path, pose_path, focal, pp, lk_params, start_frame=errorFrame - 5)